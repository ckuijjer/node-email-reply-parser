 ...
 * @type {RegExp}
 */
const SIGNATURE_REGEX = /(?:^\s*--|^\s*__|^-\w|^-- $)|(?:^Sent from my (?:\s*(?:\w|™)+){1,4}$)|(?:^Verstuurd vanaf mijn (?:\s*(?:\w|™)+){1,4}$)|(?:^={30,}$)$/;
...
const QUOTE_HEADERS_REGEX = [
    ...
    /^\s*(Op\s[\S\s]+?schreef[\S\s]+:)$/m, // Op DATE, schreef NAME <EMAIL>:
    /^\s*(Op\s[\S\s]+?schreef[\S\s]+exact\.com>)$/m, // Op DATE, schreef NAME <EMAIL>.exact.com
    /^\s*(Op\s[\S\s]+?geschreven:)$/m, // Op DATE, ... geschreven:    
